...
export default function Home() {
  ...
  return (
    <div>
      <section className="relative overflow-hidden bg-gradient-to-b from-cream/50 to-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-12 sm:pb-20">
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            <div>
              <motion.span initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="inline-flex items-center gap-2 px-4 py-1.5 bg-petal/80 text-rose text-xs font-bold rounded-full mb-5">
              ...
              <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.15 }} className="mt-7 flex flex-wrap gap-3">
                <Link to="/quiz" className="inline-flex items-center gap-2 px-8 py-3.5 bg-gradient-to-r from-mahogany to-espresso text-white text-sm font-bold rounded-full hover:from-espresso hover:to-dark-sienna transition-all shadow-2xl shadow-primary-300/30 hover:shadow-primary-400/40">
                  Find Your Shade <span className="text-lg">🎨</span>
                </Link>
                <Link to="/learn" className="inline-flex items-center gap-2 px-8 py-3.5 bg-white text-mahogany text-sm font-bold rounded-full hover:bg-cream transition-all shadow-lg border border-primary-200/60 hover:border-primary-300">
              ...
              <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }} className="mt-8 flex items-center gap-6 text-xs text-gray-500 font-medium">
                ...
                <span className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-terracotta" /> 10+ Brands</span>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }} className="relative">
              <div className="relative aspect-[4/5] rounded-3xl overflow-hidden bg-gradient-to-br from-sand via-blush-cream to-sand shadow-2xl shadow-primary-200/30">
      ...
      <section className="bg-gradient-to-b from-cream/40 to-ivory/20 py-14 sm:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            <span className="text-mahogany font-bold text-[10px] uppercase tracking-[0.2em]">How It Works</span>
          ...
          <div className="grid sm:grid-cols-3 gap-6 sm:gap-8">
            {[
              { step: "01", icon: "📝", title: "Take the Quiz", description: "Tell us about your skin tone, undertone, concerns, and finish preferences. Takes less than 2 minutes.", color: "from-sand to-cream" },
              { step: "02", icon: "🎯", title: "Get Your Matches", description: "We recommend your perfect foundation, concealer, and skin tint shades across 10+ beauty brands.", color: "from-blush-cream to-ivory" },
              ...
              <motion.div key={s.step} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="text-center p-6 sm:p-8 bg-white rounded-2xl border border-primary-100/60 hover:shadow-xl hover:shadow-primary-100/20 transition-all duration-300">
                ...
                <span className="text-[10px] font-bold text-caramel uppercase tracking-wider">{s.step}</span>
          ...
          <div className="text-center mt-10">
            <Link to="/quiz" className="inline-flex items-center gap-2 px-8 py-3.5 bg-gradient-to-r from-mahogany to-espresso text-white text-sm font-bold rounded-full hover:from-espresso hover:to-dark-sienna transition-all shadow-xl shadow-primary-300/30">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 sm:py-20">
        <div className="flex items-center justify-between mb-8">
          <div>
            <span className="text-mahogany font-bold text-[10px] uppercase tracking-[0.2em]">Supported Brands</span>
          ...
          {topBrands.map((b, i) => (
            <motion.div key={b.id} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }}>
              <Link to={`/learn#${b.id}`} className="group block p-5 bg-white rounded-2xl border border-primary-100/60 hover:border-primary-300/60 hover:shadow-xl hover:shadow-primary-100/20 transition-all duration-300 text-center">
                <div className="aspect-[4/3] rounded-xl overflow-hidden bg-gradient-to-br from-cream to-ivory mb-3">
                  <img src={b.image} alt={b.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                </div>
                <h3 className="text-sm font-bold text-gray-900 group-hover:text-mahogany transition-colors">{b.name}</h3>
                <span className="inline-block mt-1.5 px-2.5 py-0.5 bg-cream text-mahogany text-[10px] font-semibold rounded-full">{b.tag}</span>
              </Link>
            </motion.div>
          ))}
        </div>
        <div className="text-center mt-8">
          <Link to="/learn" className="text-xs sm:text-sm text-mahogany hover:text-espresso font-bold flex items-center justify-center gap-1.5 hover:gap-2 transition-all">
      ...
      <section className="bg-gradient-to-b from-ivory/30 to-cream/30 py-14 sm:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            <span className="text-mahogany font-bold text-[10px] uppercase tracking-[0.2em]">Shade Types We Cover</span>
          ...
          <div className="grid sm:grid-cols-3 gap-4 sm:gap-6">
            {[
              ...
              <motion.div key={item.type} initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="p-6 sm:p-8 bg-white rounded-2xl border border-primary-100/60 hover:shadow-xl transition-all duration-300 text-center">
                <div className="w-14 h-14 mx-auto rounded-xl bg-gradient-to-br from-sand to-blush-cream flex items-center justify-center text-2xl mb-4">{item.icon}</div>
                ...
                <p className="text-mahogany font-bold text-sm mt-3">{item.count} Products in Database</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14 sm:py-20">
        <div className="text-center mb-10">
          <span className="text-mahogany font-bold text-[10px] uppercase tracking-[0.2em]">Learn & Explore</span>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 sm:gap-6">
          {blogPosts.map((post, i) => (
            <motion.div key={post.id} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }} className="group cursor-pointer">
              <div className="aspect-[16/10] rounded-2xl overflow-hidden bg-cream mb-3 shadow-lg shadow-primary-100/10">
              ...
              <h3 className="font-bold text-gray-900 text-sm sm:text-base group-hover:text-mahogany transition-colors leading-snug">{post.title}</h3>
              <p className="text-xs text-gray-500 mt-1 line-clamp-2 leading-relaxed font-medium">{post.excerpt}</p>
            </motion.div>
          ))}
        </div>
        <div className="text-center mt-8">
          <Link to="/learn" className="inline-flex items-center gap-2 px-7 py-3 bg-white text-mahogany font-bold text-sm rounded-full hover:bg-cream transition-all shadow-lg border border-primary-200/60">
      ...
      <section className="bg-gradient-to-br from-espresso via-dark-sienna to-espresso text-white py-14 sm:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="font-serif text-3xl sm:text-4xl font-bold tracking-tight">Ready to Find Your Perfect Shade?</h2>
          <p className="text-sand mt-3 text-sm sm:text-base max-w-xl mx-auto font-medium">Join thousands of Indian women who've stopped guessing and started glowing.</p>
          <Link to="/quiz" className="mt-8 inline-flex items-center gap-2 px-8 py-3.5 bg-white text-mahogany font-bold text-sm rounded-full hover:bg-cream transition-all shadow-2xl shadow-black/10">
